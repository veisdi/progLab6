    ...
    public void run() {
        ...
        try {
            ...
            for (int attempt = 1; attempt <= MAX_RECONNECT_ATTEMPTS; attempt++) {
                ...
                System.err.println("Попытка переподключения... (Попытка " + attempt + " из " + MAX_RECONNECT_ATTEMPTS + ")");

                try {
                    Thread.sleep(2000);

                    if (!channel.isOpen()) {
                        System.err.println("Канал связи закрыт. Невозможно восстановить соединение.");
                        return;
                    }

                    System.out.println("Проверка связи с сервером (отправка команды 'info')...");
                    try {
                        Command testCommand = new lab6.server.commands.InfoCommand();
                        // сервер не отвечает - IOException
                        sendCommand(testCommand);

                        reconnected = true;
                        System.out.println("Связь восстановлена! Сервер отвечает. Продолжаем работу.");
                        break;
                    ...
                    System.err.println("Ошибка при проверке связи: " + ex.getMessage());
                }
            }
            if (!reconnected) {
                System.err.println("Не удалось подключиться к серверу после " + MAX_RECONNECT_ATTEMPTS + " попыток.");
                return;
            }
